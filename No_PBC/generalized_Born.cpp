...
void GENERALIZED_BORN_INFORMATION::Get_Effective_Born_Radius(const VECTOR *crd)
{
    if (is_initialized)
    {
        ...
        CUDA_LAUNCH(Effective_Born_Radii_Factor_CUDA,
            dim3((unsigned int)ceilf((float)atom_numbers / thread_GB.x), (unsigned int)ceilf((float)atom_numbers / thread_GB.y)),
            dim3(thread_GB.x, thread_GB.y),
            atom_numbers, crd, radii_cutoff * radii_cutoff, d_GB_self_radius, d_GB_other_radius, d_GB_effective_radius);
        CUDA_LAUNCH(Effective_Born_Radii_CUDA,
            dim3((unsigned int)ceilf((float)atom_numbers / thread_GB.x / thread_GB.y)),
            dim3(thread_GB.x * thread_GB.y),
...
float GENERALIZED_BORN_INFORMATION::Get_Energy(const VECTOR *crd, const float *charge, int is_download)
{
    if (is_initialized)
    {
        ...
        Get_Effective_Born_Radius(crd);
        CUDA_LAUNCH(GB_inej_Energy_CUDA,
            dim3((unsigned int)ceilf((float)atom_numbers / thread_GB.x), (unsigned int)ceilf((float)atom_numbers / thread_GB.y)),
            dim3(thread_GB.x, thread_GB.y),
            atom_numbers, crd, charge, d_GB_effective_radius, 1.0 / relative_dielectric_constant - 1.0, cutoff * cutoff, d_GB_energy_atom);
        CUDA_LAUNCH(GB_ieqj_Energy_CUDA,
            dim3((unsigned int)ceilf((float)atom_numbers / thread_GB.x / thread_GB.y)),
            dim3(thread_GB.x * thread_GB.y),
            atom_numbers, crd, charge, d_GB_effective_radius, 0.5 / relative_dielectric_constant - 0.5, cutoff * cutoff, d_GB_energy_atom);
