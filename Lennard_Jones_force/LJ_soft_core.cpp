...
void LJ_SOFT_CORE::Initial(CONTROLLER *controller, float cutoff, VECTOR box_length,char *module_name)
{
    ...
    if (is_initialized)
    {
        ...
        CUDA_LAUNCH(Copy_LJ_Type_And_Mask_To_New_Crd,
    ...
    const int need_virial, float *atom_lj_virial, float *atom_direct_pme_energy)
{
    if (is_initialized)
    {
        ...
        {
            CUDA_LAUNCH((Lennard_Jones_And_Direct_Coulomb_Soft_Core_CUDA<true, false, false, true, false>),
                dim3((atom_numbers + thread_LJ.y - 1) / thread_LJ.y),
                dim3(thread_LJ),
                atom_numbers, nl,
        ...
        else if (need_atom_energy && !need_virial)
        {
            ...
            CUDA_LAUNCH((Lennard_Jones_And_Direct_Coulomb_Soft_Core_CUDA<true, true, false, true, false>),
                dim3((atom_numbers + thread_LJ.y - 1) / thread_LJ.y),
                dim3(thread_LJ),
                atom_numbers, nl,
                uint_crd_with_LJ, uint_dr_to_dr_cof,
        ...
        else if (!need_atom_energy > 0 && need_virial> 0)
        {
            ...
            CUDA_LAUNCH((Lennard_Jones_And_Direct_Coulomb_Soft_Core_CUDA<true, false, true, true, false>),
                dim3((atom_numbers + thread_LJ.y - 1) / thread_LJ.y),
                dim3(thread_LJ),
                atom_numbers, nl,
                uint_crd_with_LJ, uint_dr_to_dr_cof,
        ...
        {
            cudaMemset(atom_direct_pme_energy, 0, sizeof(float) * this->atom_numbers);
            CUDA_LAUNCH((Lennard_Jones_And_Direct_Coulomb_Soft_Core_CUDA<true, true, true, true, false>),
                dim3((atom_numbers + thread_LJ.y - 1) / thread_LJ.y),
                dim3(thread_LJ),
                atom_numbers, nl,
                uint_crd_with_LJ, uint_dr_to_dr_cof,
...
float LJ_SOFT_CORE::Get_Energy(const UNSIGNED_INT_VECTOR *uint_crd, const ATOM_GROUP *nl, const float pme_beta, const float* charge, float *pme_direct_ene, int is_download)
{
    if (is_initialized)
    {
        ...
        CUDA_LAUNCH(Reset_List,
            ...
            pme_direct_ene,
            0.0f);
        CUDA_LAUNCH((Lennard_Jones_And_Direct_Coulomb_Soft_Core_CUDA<false, true, false, true, false>),
            dim3((atom_numbers + thread_LJ.y - 1) / thread_LJ.y),
            dim3(thread_LJ),
            atom_numbers, nl,
            uint_crd_with_LJ, uint_dr_to_dr_cof,
    ...
    const float *charge_B_A, const float pme_beta, const int charge_perturbated, int is_download)
{
    if (is_initialized)
    {
        ...
        if (charge_perturbated > 0)
        {
            ...
            CUDA_LAUNCH((Lennard_Jones_And_Direct_Coulomb_Soft_Core_CUDA<false, false, false, true, true>),
                dim3((atom_numbers + thread_LJ.y - 1) / thread_LJ.y),
                dim3(thread_LJ),
                atom_numbers, nl,
                uint_crd_with_LJ, uint_dr_to_dr_cof,
        ...
        {
            CUDA_LAUNCH((Lennard_Jones_And_Direct_Coulomb_Soft_Core_CUDA<false, false, false, false, true>),
                dim3((atom_numbers + thread_LJ.y - 1) / thread_LJ.y),
                dim3(thread_LJ),
                atom_numbers, nl,
                uint_crd_with_LJ, uint_dr_to_dr_cof,
