...
void ANDERSEN_THERMOSTAT_INFORMATION::Initial(CONTROLLER *controller, float target_temperature, int atom_numbers, float *h_mass, const char *module_name)
{
    ...
    {
        controller->Check_Int(this->module_name, "seed", "ANDERSEN_THERMOSTAT_INFORMATION::Initial");
    ...
    CUDA_LAUNCH(Setup_Rand_Normal_Kernel,
        dim3((unsigned int)ceilf((float)float4_numbers / 1024)),
        dim3(1024),
    ...
    {
        controller->Check_Int(this->module_name, "update_interval", "ANDERSEN_THERMOSTAT_INFORMATION::Initial");
    ...
    {
        controller->Check_Float("velocity_max", "ANDERSEN_THERMOSTAT_INFORMATION::Initial");
...
void ANDERSEN_THERMOSTAT_INFORMATION::MD_Iteration_Leap_Frog(int atom_numbers, VECTOR *vel, VECTOR *crd, VECTOR *frc, VECTOR *acc, float *inverse_mass, float dt)
{
    if (is_initialized)
    {
        ...
        if (max_velocity <= 0)
        {    
            CUDA_LAUNCH(MD_Iteration_Leap_Frog_With_Andersen,
        ...
        {
            CUDA_LAUNCH(MD_Iteration_Leap_Frog_With_Andersen_With_Max_Velocity,
                dim3((unsigned int)ceilf((float)atom_numbers / 1024)),
                dim3(1024),
