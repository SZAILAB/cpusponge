...
void BOND::Initial(CONTROLLER* controller, CONECT* connectivity, PAIR_DISTANCE* con_dis, const char* module_name)
{
    //给予bond模块一个默认名字：bond
    if (module_name == NULL)
    {
    ...
    if (is_initialized)
    {
        for (int i = 0; i < bond_numbers; i += 1)
        {
            ...
            {
                con_dis[0][std::pair<int, int>(h_atom_a[i], h_atom_b[i])] = h_r0[i];
            }
            else
            {
                con_dis[0][std::pair<int, int>(h_atom_b[i], h_atom_a[i])] = h_r0[i];
            }
        }
...
float BOND::Get_Energy(const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler, int is_download)
{
    if (is_initialized)
    {
        CUDA_LAUNCH(Bond_Energy_CUDA,
            dim3((unsigned int)ceilf((float)this->bond_numbers / this->threads_per_block)),
            dim3(static_cast<unsigned int>(this->threads_per_block)),
            this->bond_numbers,
            uint_crd,
            scaler,
            this->d_atom_a,
        ...
        CUDA_LAUNCH(Sum_Of_List, dim3(1), dim3(1024), this->bond_numbers, this->d_bond_ene, this->d_sigma_of_bond_ene);
        if (is_download)
        {
...
void BOND::Bond_Force_With_Atom_Energy_And_Virial(const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler, VECTOR *frc, float *atom_energy, float *atom_virial)
{
    if (is_initialized)
    {
        CUDA_LAUNCH(Bond_Force_With_Atom_Energy_And_Virial_CUDA,
            dim3((unsigned int)ceilf((float)this->bond_numbers / this->threads_per_block)),
            dim3(static_cast<unsigned int>(this->threads_per_block)),
            this->bond_numbers,
            uint_crd,
