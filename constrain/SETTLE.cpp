...
static inline bool Settle_Virial_Debug_On()
{
    const char* value = std::getenv("SPONGE_DEBUG_SETTLE_VIRIAL");
...
void SETTLE::Remember_Last_Coordinates(UNSIGNED_INT_VECTOR *uint_crd, VECTOR scaler)
{
    ...
    CUDA_LAUNCH(remember_pair_AB, dim3(64), dim3(1024),
        pair_numbers, d_pairs, uint_crd, scaler, last_pair_AB);
    CUDA_LAUNCH(remember_triangle_BA_CA, dim3(64), dim3(1024),
        triangle_numbers, d_triangles, uint_crd, scaler, last_triangle_BA, last_triangle_CA);
}
    ...
    int need_pressure, float* d_pressure)
{
    ...
    CUDA_LAUNCH(settle_pair, dim3(64), dim3(64),
        pair_numbers, d_pairs, d_mass, crd, box_length, last_pair_AB,
        constrain->dt, constrain->v_factor, constrain->x_factor, vel, virial_vector + triangle_numbers);
    
    CUDA_LAUNCH(settle_triangle, dim3(64), dim3(64),
        triangle_numbers, d_triangles, d_mass, crd, box_length, last_triangle_BA, last_triangle_CA,
        constrain->dt, constrain->v_factor, constrain->x_factor, vel, virial_vector);

    if (need_pressure)
    {
        const int virial_terms = triangle_numbers + pair_numbers;
        const float pressure_factor = 0.33333f / box_length.x / box_length.y / box_length.z;
        float pressure_before = 0.0f;
        double virial_sum_xyz = 0.0;
        if (Settle_Virial_Debug_On())
        {
            cudaMemcpy(&pressure_before, d_pressure, sizeof(float), cudaMemcpyDeviceToHost);
            std::vector<VECTOR> host_virial(static_cast<size_t>(virial_terms));
            cudaMemcpy(host_virial.data(), virial_vector, sizeof(VECTOR) * static_cast<size_t>(virial_terms), cudaMemcpyDeviceToHost);
            for (int i = 0; i < virial_terms; i += 1)
            {
                virial_sum_xyz += static_cast<double>(host_virial[static_cast<size_t>(i)].x);
                virial_sum_xyz += static_cast<double>(host_virial[static_cast<size_t>(i)].y);
                virial_sum_xyz += static_cast<double>(host_virial[static_cast<size_t>(i)].z);
            }
        }
        CUDA_LAUNCH(Sum_Of_Virial_Vector_To_Pressure, dim3(64), dim3(1024),
            virial_terms, virial_vector, d_pressure, pressure_factor);
        if (Settle_Virial_Debug_On())
        {
            ...
            debug_call += 1;
            std::printf(
                "DEBUG_SETTLE_VIRIAL call=%d terms=%d virial_sum_xyz=%e factor=%e pressure_before=%e pressure_after=%e pressure_delta=%e expected_delta=%e\n",
                debug_call,
                virial_terms,
                virial_sum_xyz,
                pressure_factor,
                ...
                virial_sum_xyz * static_cast<double>(pressure_factor));
            std::fflush(stdout);
        }
    }
}
