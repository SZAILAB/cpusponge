...
#define CUDA_LAUNCH(kernel, grid, block, ...) \
    cpu_launch_kernel((grid), (block), [&]() { kernel(__VA_ARGS__); })
#define CUDA_LAUNCH_SHARED(kernel, grid, block, shared_bytes, ...) \
    cpu_launch_kernel((grid), (block), [&]() { kernel(__VA_ARGS__); })
#define CUDA_LAUNCH_STREAM(kernel, grid, block, shared_bytes, stream, ...) \
    cpu_launch_kernel((grid), (block), [&]() { kernel(__VA_ARGS__); })
#else
#define CUDA_LAUNCH(kernel, grid, block, ...) \
    kernel<<<(grid), (block)>>>(__VA_ARGS__)
#define CUDA_LAUNCH_SHARED(kernel, grid, block, shared_bytes, ...) \
    kernel<<<(grid), (block), (shared_bytes)>>>(__VA_ARGS__)
#define CUDA_LAUNCH_STREAM(kernel, grid, block, shared_bytes, stream, ...) \
    kernel<<<(grid), (block), (shared_bytes), (stream)>>>(__VA_ARGS__)
#endif
...
使用样例：（均在No_PNC/generalized_Born.cpp中）
...
可以直接将下面的代码和common.h复制到一个新文件test.cpp，再使用nvcc test.cpp -o SAD_TEST编译，最后使用./SAD_TEST运行
