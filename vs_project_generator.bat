...
for /F "tokens=5" %%i in ('nvcc -V ^| findstr release') do ( set CUDA=%%i)
set CUDA=%CUDA:~0,-1%
echo Find CUDA version %CUDA%
if %CUDA% GEQ 11.0 (
set CodeGeneration=compute_52,sm_52
set AdditionalCompilerOptions=-DCUDA_ARCH_BIN=52
) else (
set CodeGeneration=compute_50,sm_50
set AdditionalCompilerOptions=-DCUDA_ARCH_BIN=50
)
echo set CUDA CodeGeneration to %CodeGeneration%
echo.
...
set i=2
set "exe="
echo    Writing SPONGE.sln
echo Microsoft Visual Studio Solution File, Format Version 12.00> SPONGE.sln
echo # Visual Studio 2013>> SPONGE.sln
echo VisualStudioVersion = 12.0.21005.1>> SPONGE.sln
echo MinimumVisualStudioVersion = 10.0.40219.1>> SPONGE.sln
for %%o in (%all_exe%) do (
    set "exe=%%o"
    echo Project^("{8BC9CEB8-8B4A-11D0-8D11-00A0C91BC942}"^) = "!exe!", "!exe!.vcxproj", "{28E30FB4-9F17-4F20-985C-FF9E3F4E8B7!i!}">> SPONGE.sln
    echo EndProject>> SPONGE.sln
    set /a i=!i!+1)
echo Global>> SPONGE.sln
echo 	GlobalSection^(SolutionConfigurationPlatforms^) = preSolution>> SPONGE.sln
echo 		Release^|x64 = Release^|x64>> SPONGE.sln
echo 	EndGlobalSection>> SPONGE.sln
echo 	GlobalSection^(ProjectConfigurationPlatforms^) = postSolution>> SPONGE.sln
set i=2
for %%o in (%all_exe%) do (
    echo 		{28E30FB4-9F17-4F20-985C-FF9E3F4E8B7!i!}.Release^|x64.ActiveCfg = Release^|x64>> SPONGE.sln
    echo 		{28E30FB4-9F17-4F20-985C-FF9E3F4E8B7!i!}.Release^|x64.Build.0 = Release^|x64>> SPONGE.sln
    set /a i=!i!+1)
echo 	EndGlobalSection>> SPONGE.sln
echo 	GlobalSection^(SolutionProperties^) = preSolution>> SPONGE.sln
echo 		HideSolutionNode = FALSE>> SPONGE.sln
echo 	EndGlobalSection>> SPONGE.sln
echo EndGlobal>> SPONGE.sln
echo.
...
for %%o in (%objects%) do (
    if !v!==0 (
        ...
        echo     ^<ProjectGuid^>{28E30FB4-9F17-4F20-985C-FF9E3F4E8B7!j!}^</ProjectGuid^>>> !exe!.vcxproj
        ...
        echo     ^<Import Project="$(VCTargetsPath)\BuildCustomizations\CUDA %CUDA%.props" /^>>> !exe!.vcxproj
        ...
        echo       ^<AdditionalDependencies^>nvrtc.lib;cudadevrt.lib;cufft.lib;cudart_static.lib;cuda.lib;kernel32.lib;user32.lib;gdi32.lib;winspool.lib;comdlg32.lib;advapi32.lib;shell32.lib;ole32.lib;oleaut32.lib;uuid.lib;odbc32.lib;odbccp32.lib;%%^(AdditionalDependencies^)^</AdditionalDependencies^>>> !exe!.vcxproj
        echo     ^</Link^>>> !exe!.vcxproj
        ...
        echo       ^<CodeGeneration^>%CodeGeneration%^</CodeGeneration^>>> !exe!.vcxproj
        echo       ^<AdditionalCompilerOptions^>%AdditionalCompilerOptions%^</AdditionalCompilerOptions^>>> !exe!.vcxproj
        echo     ^</CudaCompile^>>> !exe!.vcxproj
        ...
        for %%a in (!all_objects!) do (
            set temp=%%a
            set "temp=!temp:/=\!"
            echo     ^<CudaCompile Include="!temp:.o=.cpp!" /^>>> !exe!.vcxproj
            echo     ^<ClInclude Include="!temp:.o=.h!" /^>>> !exe!.vcxproj
        )
        echo   ^</ItemGroup^>>> !exe!.vcxproj
        echo   ^<Import Project="$(VCTargetsPath)\Microsoft.Cpp.targets" /^>>> !exe!.vcxproj
        echo   ^<ImportGroup Label="ExtensionTargets"^>>> !exe!.vcxproj
        echo     ^<Import Project="$(VCTargetsPath)\BuildCustomizations\CUDA %CUDA%.targets" /^>>> !exe!.vcxproj
        echo   ^</ImportGroup^>>> !exe!.vcxproj
        ...
        for %%a in (!all_objects!) do (
            ...
            if !tttemp!==0 (
                echo     ^<CudaCompile Include="!temp:.o=.cpp!" /^>>> !exe!.vcxproj.filters
                echo     ^<ClInclude Include="!temp:.o=.h!" /^>>> !exe!.vcxproj.filters
            ) else (
                echo     ^<CudaCompile Include="!temp:.o=.cpp!"^>>> !exe!.vcxproj.filters
