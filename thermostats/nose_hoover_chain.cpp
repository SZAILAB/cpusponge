...
VECTOR *frc, VECTOR *acc, float chain_vel, float max_vel)
{
    ...
    if (i < atom_numbers)
    {
        ...
        vel[i] = Make_Vector_Not_Exceed_Value(vel[i], max_vel);
...
void NOSE_HOOVER_CHAIN_INFORMATION::Initial(CONTROLLER *controller, float target_temperature, const char *module_name)
{
    ...
    {
        controller->Check_Int(this->module_name, "length", "NOSE_HOOVER_CHAIN_INFORMATION::Initial");
    ...
    {
        controller->Check_Float(this->module_name, "tau", "NOSE_HOOVER_CHAIN_INFORMATION::Initial");
        tauT = atof(controller->Command(this->module_name, "tau"));
    }
    ...
    if (controller[0].Command_Exist(this->module_name, "crd"))
    {
        ...
        controller->Set_File_Buffer(f_crd_traj, sizeof(char) * 15 * chain_length);
    }
    tempchar[0] = 0;
    f_vel_traj = NULL;
    if (controller[0].Command_Exist(this->module_name, "vel"))
    {
        ...
        controller->Set_File_Buffer(f_vel_traj, sizeof(char) * 15 * chain_length);
    }

    max_velocity = 0;
    if (controller[0].Command_Exist("velocity_max"))
    {
        controller->Check_Float(this->module_name, "velocity_max", "NOSE_HOOVER_CHAIN_INFORMATION::Initial");
...
void NOSE_HOOVER_CHAIN_INFORMATION::MD_Iteration_Leap_Frog(int atom_numbers, VECTOR *vel, VECTOR *crd, VECTOR *frc, VECTOR *acc, float *inverse_mass, float dt, float Ek, int freedom)
{
    if (is_initialized)
    {
        ...
        if (max_velocity <= 0)
        {
            CUDA_LAUNCH(MD_Iteration_Leap_Frog_With_NHC,
                dim3((unsigned int)ceilf((float)atom_numbers / 1024)),
                dim3(1024),
                atom_numbers, dt, inverse_mass, vel, crd, frc, acc, velocity[0]);
        }
        else
        {
            CUDA_LAUNCH(MD_Iteration_Leap_Frog_With_NHC_With_Max_Velocity,
                dim3((unsigned int)ceilf((float)atom_numbers / 1024)),
                dim3(1024),
