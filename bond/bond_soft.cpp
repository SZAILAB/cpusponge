...
void BOND_SOFT::Initial(CONTROLLER *controller, const char *module_name)
{
    ...
    if (controller[0].Command_Exist(this->module_name, "in_file"))
    {
        ...
        {
            controller->Throw_SPONGE_Error(spongeErrorMissingCommand, "BOND_SOFT::Initial",
                "Reason:\n\t'lambda_bond' is required for the calculation of bond_soft.\n");
        }
...
void BOND_SOFT::Soft_Bond_Force_With_Atom_Energy_And_Virial(const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler, VECTOR *frc, float * atom_energy, float *atom_virial)
{
    if (is_initialized)
    {
        CUDA_LAUNCH(Soft_Bond_Force_With_Atom_Energy_And_Virial_CUDA,
            dim3((unsigned int)ceilf((float)this->soft_bond_numbers / this->threads_per_block)),
...
float BOND_SOFT::Get_Energy(const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler, int is_download)
{
    if (is_initialized)
    {
        CUDA_LAUNCH(Soft_Bond_Energy_CUDA,
            dim3((unsigned int)ceilf((float)this->soft_bond_numbers / this->threads_per_block)),
            dim3(this->threads_per_block),
            this->soft_bond_numbers, uint_crd, scaler,
        ...
        CUDA_LAUNCH(Sum_Of_List, dim3(1), dim3(1024),
            this->soft_bond_numbers, this->d_soft_bond_ene, this->d_sigma_of_soft_bond_ene);
        if (is_download)
        {
...
float BOND_SOFT::Get_Partial_H_Partial_Lambda(const UNSIGNED_INT_VECTOR * uint_crd, const VECTOR scaler, int is_download)
{
    if (is_initialized)
    {
        CUDA_LAUNCH(Soft_Bond_dH_dlambda_CUDA,
            dim3((unsigned int)ceilf((float)this->soft_bond_numbers / this->threads_per_block)),
            dim3(this->threads_per_block),
            this->soft_bond_numbers, uint_crd, scaler,
        ...
        CUDA_LAUNCH(Sum_Of_List, dim3(1), dim3(1024),
            this->soft_bond_numbers, this->d_soft_bond_dH_dlambda, this->d_sigma_of_dH_dlambda);
        if (is_download)
        {
