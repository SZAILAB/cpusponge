...
float DIHEDRAL::Get_Energy(const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler, int is_download)
{
    if (is_initialized)
    {
        CUDA_LAUNCH(Dihedral_Energy_CUDA,
            dim3((unsigned int)ceilf((float)this->dihedral_numbers / this->threads_per_block)),
            ...
            this->d_pn,
            this->d_dihedral_ene);
        CUDA_LAUNCH(Sum_Of_List, dim3(1), dim3(1024), this->dihedral_numbers, this->d_dihedral_ene, this->d_sigma_of_dihedral_ene);
...
void DIHEDRAL::Dihedral_Force_With_Atom_Energy(const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler, VECTOR *frc, float *atom_energy)
{
    if (is_initialized)
    {
        CUDA_LAUNCH(Dihedral_Force_With_Atom_Energy_CUDA,
            dim3((unsigned int)ceilf((float)this->dihedral_numbers / this->threads_per_block)),
            dim3(static_cast<unsigned int>(this->threads_per_block)),
            this->dihedral_numbers,
            uint_crd,
