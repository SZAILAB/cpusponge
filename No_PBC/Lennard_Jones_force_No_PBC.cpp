    ...
    const int *excluded_list_start, const int *excluded_list, const int *excluded_atom_numbers)
{
    if (is_initialized)
    {
        ...
        {
            CUDA_LAUNCH(LJ_Force_CUDA,
                dim3((unsigned int)ceilf((float)atom_numbers / thread_LJ.x), (unsigned int)ceilf((float)atom_numbers / thread_LJ.y)),
                dim3(thread_LJ.x, thread_LJ.y),
        ...
        {
            CUDA_LAUNCH(LJ_Force_Energy_CUDA,
                dim3((unsigned int)ceilf((float)atom_numbers / thread_LJ.x), (unsigned int)ceilf((float)atom_numbers / thread_LJ.y)),
                dim3(thread_LJ.x, thread_LJ.y),
                atom_numbers, crd, d_atom_LJ_type, d_LJ_A, d_LJ_B,
                excluded_list_start, excluded_list, excluded_atom_numbers,
