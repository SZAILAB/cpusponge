...
{    
    ...
    if (controller[0].Command_Exist(this->module_name, file_name_suffix))
    {
        ...
        {
            controller->Throw_SPONGE_Error(spongeErrorConflictingCommand, "NON_BOND_14::Initial", "Reason:\n\t'nb14_in_file' should be provided with initialized LJ parameters\n");
        }
...
float NON_BOND_14::Get_14_LJ_Energy(const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler, int is_download)
{
    if (is_initialized)
    {
        CUDA_LAUNCH(Dihedral_14_LJ_Energy,
            dim3((unsigned int)ceilf((float)nb14_numbers / threads_per_block)),
            ...
            d_B,
            d_nb14_energy);
        CUDA_LAUNCH(Sum_Of_List, dim3(1), dim3(1024), nb14_numbers, d_nb14_energy, d_nb14_lj_energy_sum);
        if (is_download)
        {
...
float NON_BOND_14::Get_14_CF_Energy(const UNSIGNED_INT_VECTOR *uint_crd, const float *charge, const VECTOR scaler, int is_download)
{
    if (is_initialized)
    {
        CUDA_LAUNCH(Dihedral_14_CF_Energy,
            dim3((unsigned int)ceilf((float)nb14_numbers / threads_per_block)),
            dim3(static_cast<unsigned int>(threads_per_block)),
            nb14_numbers,
            uint_crd,
            ...
            d_cf_scale_factor,
            d_nb14_energy);
        CUDA_LAUNCH(Sum_Of_List, dim3(1), dim3(1024), nb14_numbers, d_nb14_energy, d_nb14_cf_energy_sum);
        if (is_download)
        {
...
void NON_BOND_14::Non_Bond_14_LJ_CF_Force_With_Atom_Energy_And_Virial(const UNSIGNED_INT_VECTOR *uint_crd, const float *charge, const VECTOR scaler, VECTOR *frc, float *atom_energy, float *atom_virial)
{
    if (is_initialized)
    {
        CUDA_LAUNCH(Dihedral_14_LJ_CF_Force_With_Atom_Energy_And_Virial_Cuda,
            dim3((unsigned int)ceilf((float)nb14_numbers / threads_per_block)),
            dim3(static_cast<unsigned int>(threads_per_block)),
            nb14_numbers,
            uint_crd,
