...
void MIDDLE_Langevin_INFORMATION::Initial(CONTROLLER *controller, const int atom_numbers, const float target_temperature, const float *h_mass, const char *module_name)
{
    ...
    {
        controller->Check_Float(this->module_name, "gamma", "MIDDLE_Langevin_INFORMATION::Initial");
    ...
    {
        controller->Check_Int(this->module_name, "seed", "MIDDLE_Langevin_INFORMATION::Initial");
...
void MIDDLE_Langevin_INFORMATION::MD_Iteration_Leap_Frog(VECTOR *frc, VECTOR *vel, VECTOR *acc, VECTOR *crd)
{
    if (is_initialized)
    {
        CUDA_LAUNCH(Rand_Normal,
            dim3(static_cast<unsigned int>(ceilf((float)float4_numbers / 32.))),
            dim3(32),
            float4_numbers,
            rand_state,
            (float4*)random_force);
        ...
        if (max_velocity <= 0)
        {
            CUDA_LAUNCH(MD_Iteration_Leap_Frog_With_LiuJian,
        ...
        {
            CUDA_LAUNCH(MD_Iteration_Leap_Frog_With_LiuJian_With_Max_Velocity,
                dim3(static_cast<unsigned int>(ceilf((float)atom_numbers / 32))),
                dim3(32),
                atom_numbers,
                half_dt,
