...
void Langevin_MD_INFORMATION::Initial(CONTROLLER *controller,const int atom_numbers, const float target_temperature,const float *h_mass, const char *module_name)
{
    ...
    {
        controller->Check_Float(this->module_name, "gamma", "Langevin_MD_INFORMATION::Initial");
    ...
    {
        controller->Check_Int(this->module_name, "seed", "Langevin_MD_INFORMATION::Initial");
    ...
    CUDA_LAUNCH(Setup_Rand_Normal_Kernel,
        dim3((unsigned int)ceilf((float)float4_numbers / threads_per_block)),
    ...
    {
        controller->Check_Float("velocity_max", "Langevin_MD_INFORMATION::Initial");
...
void Langevin_MD_INFORMATION::MD_Iteration_Leap_Frog(VECTOR *frc, VECTOR *crd, VECTOR *vel, VECTOR *acc)
{
    if (is_initialized)
    {
        ...
        if (max_velocity <= 0)
        {
            CUDA_LAUNCH(MD_Iteration_Leap_Frog_With_Langevin,
                dim3((unsigned int)ceilf((float)atom_numbers / threads_per_block)),
                dim3(threads_per_block),
                atom_numbers, dt, d_mass_inverse, gamma_ln, d_sigma_mass, vel, crd, frc, acc, random_force);
        }
        else
        {
            CUDA_LAUNCH(MD_Iteration_Leap_Frog_With_Langevin_With_Max_Velocity,
                dim3((unsigned int)ceilf((float)atom_numbers / threads_per_block)),
                dim3(threads_per_block),
