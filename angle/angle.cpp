...
void ANGLE::Read_Information_From_AMBERFILE(const char *file_name, CONTROLLER controller)
{
    ...
    {
        controller.Throw_SPONGE_Error(spongeErrorBadFileFormat, "ANGLE::Read_Information_From_AMBERFILE",
...
float ANGLE::Get_Energy(const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler, int is_download)
{
    if (is_initialized)
    {
        CUDA_LAUNCH(Angle_Energy_CUDA,
            dim3((unsigned int)ceilf((float)this->angle_numbers / this->threads_per_block)),
            ...
            this->d_angle_theta0,
            this->d_angle_ene);
        CUDA_LAUNCH(Sum_Of_List, dim3(1), dim3(1024), this->angle_numbers, this->d_angle_ene, this->d_sigma_of_angle_ene);
        if (is_download)
        {
...
void ANGLE::Angle_Force_With_Atom_Energy(const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler, VECTOR *frc,float *atom_energy)
{
    if (is_initialized)
    {
        CUDA_LAUNCH(Angle_Force_With_Atom_Energy_CUDA,
            dim3((unsigned int)ceilf((float)this->angle_numbers / this->threads_per_block)),
            dim3(static_cast<unsigned int>(this->threads_per_block)),
            this->angle_numbers,
            uint_crd,
